...
)
{
	...
	difference()
	{
		intersection()
		{
			...
			cylinder(r=corner_radius, h=totalht+0.2, $fa=0.2,$fs=0.2);
		}
		...
		if (esd > 0)
		{  // add pockets for screws if requested
			gridcopycorners
			(
				ceil(num_x),
				ceil(num_y),
			...
			translate([0, 0, -0.1])
			cylinder(d=screw_hole_diam, h=esd+0.1, $fa=0.2,$fs=0.2);
		}

		if (emd > 0)
		{  // add pockets for magnets if requested
			gridcopycorners
			(
				ceil(num_x),
				ceil(num_y),
...
(
    ...
    num_y=1,
    margins=0,
	//a margin added to the first element, meant to improve mating of the female part, otherwise it touches the bottom and doesn't mate well
	imz_depth=0.0
)
{
    ...
    translate([0, 0, -axialdown+imz_depth])
    difference()
    {
        union()
        {
			...
			cornercopy(pad_corner_position, num_x, num_y)
			{
				...
				translate([0, 0, bevel1_top-imz_depth])
				cylinder
				(
					d=3.2+2*radialgap,
					h=1.9+imz_depth,
			...
			cornercopy(pad_corner_position, num_x, num_y)
            {
				...
				(
					d1=3.2+2*radialgap,
					d2=7.5+0.5+2*radialgap+2*bonus_ht,
					h=bevel2_top-bevel2_bottom+bonus_ht,
					$fa=0.2,
					$fs=0.2
				);
            }
